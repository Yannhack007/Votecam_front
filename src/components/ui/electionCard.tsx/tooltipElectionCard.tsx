...
const TooltipElectionCard: React.FC<TooltipProps> = React.memo(
  ({ hoverInfo, setHoverInfo }) => {
    if (!hoverInfo.region) return null;

    const displayDataMemoized = useMemo(() => {
      if (hoverInfo.region) {
        return displayData(hoverInfo.region); // Only run the computation if there's a region
      }
      return []; // Return an empty array or default value if no region is available
    }, [hoverInfo.region]); // Add
    ...
    return (
      ...
      >
        ...
        <table className="w-full">
          ...
          <tbody>
            {displayDataMemoized.map((voteData, index) => {
...
TooltipElectionCard.displayName = 'TooltipElectionCard';
